...
def update_config_yaml():
    """
    1. If 'openai' parent key doesn't exist, creates it
    2. For each child key under 'openai', adds only if missing:
       - api_key: empty string
       - model: 'gpt-4o-mini'
       - reasoning_effort: 'medium'

    3. If 'server' parent key doesn't exist, creates it
    4. For each child key under 'server', adds only if missing:
       - api_key: empty string
       - connection_str: 'http://localhost:1234/v1'
       - show_thinking: 'medium'

    5. Removes any unauthorized child keys from the 'server' mapping,
