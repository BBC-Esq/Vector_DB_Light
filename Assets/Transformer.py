...
# Adds debugging with .json output of problematic items
...
class Transformer(InputModule):
    ...
    def tokenize(
        ...
        logger.warning(f"[TOKENIZE DEBUG] Incoming texts length: {len(texts)}")
        logger.warning(f"[TOKENIZE DEBUG] First item type: {type(texts[0])}")
        
        # Check for nested lists in incoming data
        for idx, item in enumerate(texts):  # Check ALL items now
            if isinstance(item, list):
                logger.error(f"[TOKENIZE DEBUG] NESTED LIST at index {idx}: {item[:100] if len(item) > 100 else item}")
        # === UPDATED LOGGING - END ===
        ...
        # === ADD THIS NEW LOGGING - START ===
        logger.warning(f"[TOKENIZE DEBUG] to_tokenize structure: len={len(to_tokenize)}, first_batch_len={len(to_tokenize[0]) if to_tokenize else 0}")
        
        # Save the ENTIRE batch to file before tokenization
        try:
            with open('D:/debug_full_batch.json', 'a', encoding='utf-8') as f:
                f.write(f"\n\n{'='*80}\nBATCH ABOUT TO TOKENIZE:\n")
                for batch_idx, batch in enumerate(to_tokenize):
                    f.write(f"\n--- Batch {batch_idx} ({len(batch)} items) ---\n")
                    for item_idx, item in enumerate(batch):
                        f.write(f"\nItem {item_idx}:\n")
                        f.write(f"  Type: {type(item)}\n")
                        f.write(f"  Length: {len(item) if hasattr(item, '__len__') else 'N/A'}\n")
                        f.write(f"  Content: {repr(item[:200])}...\n" if len(str(item)) > 200 else f"  Content: {repr(item)}\n")
                        
                        # Check for weird characters or empty strings
                        if not item or not item.strip():
                            f.write(f"  WARNING: Empty or whitespace-only string!\n")
                        if not isinstance(item, str):
                            f.write(f"  ERROR: NOT A STRING! Type is {type(item)}\n")
        except Exception as e:
            logger.error(f"[TOKENIZE DEBUG] Failed to write debug file: {e}")
        # === ADD THIS NEW LOGGING - END ===
