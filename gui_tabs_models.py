...
class VectorModelsTab(QWidget):
    def __init__(self, parent=None):
       ...
       vector_models_dir = config.vector_models_dir
       ...
       for vendor, models in VECTOR_MODELS.items():
           ...
           for model in models:
               ...
               radiobutton.setProperty("model_dir", expected_dir_name)
               ...
               self.downloaded_labels[f"{vendor}/{model['name']}"] = downloaded_label
    ...
    def initiate_model_download(self):
        btn = self.model_radiobuttons.checkedButton()
        if not btn:
            return
        model_info = btn.property("model_info")
        if not model_info:
            return
        model_downloader = ModelDownloader(model_info, model_info['type'])
        threading.Thread(target=lambda: model_downloader.download_model()).start()
       ...
       for vendor, models in VECTOR_MODELS.items():
           for model in models:
               if model['cache_dir'] == model_name:
                   downloaded_label = self.downloaded_labels.get(f"{vendor}/{model['name']}")
                   if downloaded_label:
                       downloaded_label.setText('Yes')
                       for button in self.model_radiobuttons.buttons():
                           if button.property("model_dir") == model_name:
