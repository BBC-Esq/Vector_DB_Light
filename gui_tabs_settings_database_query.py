...
class DatabaseSettingsTab(QWidget):
    def __init__(self):
        ...
        try:
            ...
            self.database_query_device = config.Compute_Device.database_query
            self.search_term = config.database.search_term
            self.document_type = config.database.document_types
        ...
        similarity_value = self.database_config.similarity
        ...
        contexts_value   = self.database_config.contexts
        ...
        self.search_term_edit.setPlaceholderText("Terms required...")
    ...
    def update_config(self):
        ...
        current_query_device = getattr(getattr(config, "Compute_Device", object()), "database_query", "")
        current_similarity = getattr(getattr(config, "database", object()), "similarity", 0.0)
        current_contexts = getattr(getattr(config, "database", object()), "contexts", 1)
        current_search_term = getattr(getattr(config, "database", object()), "search_term", "")
        current_document_types = getattr(getattr(config, "database", object()), "document_types", "")

        new_query_device = self.query_device_combo.currentText()
        device_changed = new_query_device != current_query_device
        ...
        if device_changed:
            config.Compute_Device.database_query = new_query_device
            settings_changed = True

        if new_similarity_text and new_similarity != current_similarity:
            config.database.similarity = new_similarity
            settings_changed = True

        if new_contexts_text and new_contexts != current_contexts:
            config.database.contexts = new_contexts
            settings_changed = True

        if new_search_term and new_search_term != current_search_term:
            config.database.search_term = new_search_term
            settings_changed = True

        if document_type_value != current_document_types:
            config.database.document_types = document_type_value
            settings_changed = True
        ...
        if settings_changed:
            ...
            if device_changed:
                self.database_query_device = config.Compute_Device.database_query
                self.query_device_label.setText(f"Device: {self.database_query_device}")

            if new_similarity_text:
                self.similarity_label.setText(f"Similarity: {self.database_config.similarity}")

            if new_contexts_text:
                self.contexts_label.setText(f"Contexts: {self.database_config.contexts}")

            if new_search_term:
                self.search_term = self.database_config.search_term
                self.search_term_label.setText(f"Search Term Filter: {self.search_term}")

            self.document_type = self.database_config.document_types
